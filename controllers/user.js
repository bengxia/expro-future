...
/**
 * 根据手机号码查询用户信息，并返回json对象。
 * @param req
 * @param res
 * @param next
 */
exports.getUserByCellphone = function(req,res,next){
    var cellphone = req.params.cellphone;
    if(!cellphone) res.json({status:400, error:'手机号码不能为空！'});
    get_user_by_loginname(cellphone, function(err, user){
        if(err) return next(err);
        if(!user || user == "undefined")  return res.json({status:204, error:'查询结果为空！'});
...
exports.checkUser = function(req,res,next){
    ...
    if(!cellphone) res.json({status:400, error:'手机号码不能为空！'});
    if(!password) res.json({status:400, error:'密码不能为空！'});
    get_user_by_loginname(cellphone, function(err, user){
        if(err) return next(err);
        if(!user || user == "undefined")  return res.json({status:204, error:'查询结果为空！'});

        //验证用户密码
        if (!bcrypt.compareSync(password, user.password)) return res.json({status:401, error:'密码错误！'});
        if (!user.state) return res.json({status:403, error:'此帐号还没有被激活！'});
